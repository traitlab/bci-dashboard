...
def candidates_panel(*, recs, n_scored, gen_n, gen_none):
    ...
    return panel(
        ...
        f'has. The {top} is an inherited <code>config.yaml</code> value '
        f'(<code>identify_nb_results</code>) with no recorded reason behind it, a setting to '
        ...
        + f'<p class="note">{full:,} of {len(recs):,} photos came back with a full {top} '
          ...
          f'the {full:,} full lists ({pctf(half / full)}) more than half of it does.</p>'
